...
function fetch_app_shopinfo(callback){
...
function fetch_shop_info(query, callback){
	console.log("查询城市："+query.City);
	async.waterfall([
		function(done){
			//去数据库中取出当前城市所有的店铺,之前dbutil的queryData方法没有释放连接池，导致每次只能取10个城市的数据
			//然后就不工作了，fuck!
			dbutil.queryData('tbl_tuhu_all_shops2',query,function(err,result){ //
				var shop_info_urls = [];
				console.log('query shops 数量：' + result.length);
			...
			console.log('urls 数量:'+ shop_info_urls.length);
			LogFile.info('店铺数量：' + shop_info_urls.length);
			var BYs = 0;//保养
		    var FWs = 0;//服务
		    var TRs = 0;//轮胎
		    var MRs = 0;//美容
			...
			async.mapSeries(shop_info_urls,function(url,callback){
				...
				request(options,function (error, response, body) {
				  if (!error && response.statusCode == 200) {
				    ...
				    if (code == 1) {
				    	...
				    	// LogFile.error(body);
				    	callback(null,{TR:0,BY:0,MR:0,FW:0});
				    }
				});    
				...
				console.log(cityName + '：'+ JSON.stringify(_.last(result)));
...
function fetch_city_shops(callback) {
	async.waterfall([
			function(done) {
				exist_shop_ids = [];
...
exports.fetch_city_shops = fetch_city_shops;
